...
public class InWindow
{
	...
	public void MoveBlock()
	{
                int offset =0;
                if(first){
                    offset = 449 + _pos - _keepSizeBefore;
                    first = false;
                }
                else
                    offset = _bufferOffset + _pos - _keepSizeBefore;
	...
	public void Create(int keepSizeBefore, int keepSizeAfter, int keepSizeReserv)
	{
		...
		if (_bufferBase == null || _blockSize != blockSize)
		{
			...
			_bufferBase = new byte[_blockSize];
                        String x = Arrays.toString(_patterns);
                        _bufferOffset = x.length();
                        System.arraycopy(x.getBytes(), 0, _bufferBase, 0, x.length());
